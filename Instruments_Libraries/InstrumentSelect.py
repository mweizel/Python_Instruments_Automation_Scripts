...
def find_resource(
    model_regex: str, resource_filter: str = "?*INSTR", specific_address: str | None = None
) -> str:
    """
    ...
    for res in resources:
        try:
            ...
            if (
                "ASRL" in res and "AME" not in model_regex
            ):  # Skip COM ports unless looking for serial devices?
            ...
            with rm.open_resource(res) as inst:
                ...
                try:
                    ...
                    if idn is None:
                        ...
                        if callable(write_func) and callable(read_func):
                            write_func("*IDN?")
                            idn = read_func().strip()
                except Exception:
                    pass
    ...
    if resource is None:
        ...
        for port in ports:
            ...
            try:
                ...
                # `port` in `for port, desc, hwid in sorted(ports)` is just the device name
                # e.g. "COM3"
